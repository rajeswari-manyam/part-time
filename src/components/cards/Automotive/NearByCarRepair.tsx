...
// Phone numbers map for car repair services
const PHONE_NUMBERS_MAP: { [key: string]: string } = {
    car_repair_1: "07942689898",
    car_repair_2: "07947428737",
    car_repair_3: "07942687166",
    car_repair_4: "07947425558",
};

// Export dummy car repair data
export const DUMMY_CAR_REPAIRS: CarRepairService[] = [
    {
        place_id: "car_repair_1",
        name: "Mody Ford",
        vicinity: "Old Bowenpally-Bowenpally, Hyderabad",
        rating: 4.4,
        user_ratings_total: 3805,
        photos: [{ photo_reference: "car_repair_photo_1" }],
        geometry: {
    ...
    {
        place_id: "car_repair_2",
        name: "Nexa Service Autofin",
        vicinity: "Bowenpally, Hyderabad",
        rating: 4.1,
        user_ratings_total: 3690,
        photos: [{ photo_reference: "car_repair_photo_2" }],
        geometry: {
            location: { lat: 17.4745, lng: 78.4515 },
        },
        business_status: "OPERATIONAL",
        opening_hours: { open_now: true },
        price_level: 2,
    ...
    {
        place_id: "car_repair_3",
        name: "Malik Cars (TATA Motors)",
        vicinity: "Road No 7 Bowenpally, Hyderabad",
        rating: 4.4,
        user_ratings_total: 2111,
        photos: [{ photo_reference: "car_repair_photo_3" }],
        geometry: {
            location: { lat: 17.4752, lng: 78.4522 },
        },
        business_status: "OPERATIONAL",
        opening_hours: { open_now: true },
        price_level: 2,
    ...
    {
        place_id: "car_repair_4",
        name: "Adishwar Auto Diagnostics Pvt Ltd",
        vicinity: "Medchal Highway Bowenpally, Hyderabad",
        rating: 4.5,
        user_ratings_total: 960,
        photos: [{ photo_reference: "car_repair_photo_4" }],
        geometry: {
            location: { lat: 17.4760, lng: 78.4530 },
        },
        business_status: "OPERATIONAL",
        opening_hours: { open_now: true },
        price_level: 2,
        types: ["car_repair", "car_service"],
        special_tags: [
            "Trending",
            "Oil Change",
            "Car Detailing",
...
// Car repair images
const CAR_REPAIR_IMAGES_MAP: { [key: string]: string[] } = {
    car_repair_1: [
...
// Car repair descriptions
const CAR_REPAIR_DESCRIPTIONS_MAP: { [key: string]: string } = {
    car_repair_1:
        "Popular car repair service with 4.4★ rating and 3,805 reviews. Company Authorised Ford Dealer. On Site Services available. Professional car maintenance and repair.",
    car_repair_2:
        "Top-rated Nexa service center with 4.1★ rating and 3,690 reviews. Verified Company Authorised Dealer. Loan Facility available. Expert Maruti Suzuki service.",
    car_repair_3:
        "Trending TATA Motors dealership with 4.4★ rating and 2,111 reviews. DoorStep Service available. Special offers on car servicing. Authorized TATA service center.",
    car_repair_4:
        "Trending auto diagnostics center with 4.5★ rating and 960 reviews. Oil Change and Car Detailing services. Quick Service available. Professional auto care.",
};

// Car repair services offered
const CAR_REPAIR_SERVICES = [
    "Oil Change",
    "Brake Service",
    "Engine Diagnostics",
    "AC Repair",
...
interface NearbyCarRepairCardProps {
    job?: any;
    onViewDetails: (job: any) => void;
}

const SingleCarRepairCard: React.FC<NearbyCarRepairCardProps> = ({
    job,
    onViewDetails,
}) => {
    ...
    const getPhotos = useCallback(() => {
        if (!job) return [];
        const jobId = job.id || "car_repair_1";
        return CAR_REPAIR_IMAGES_MAP[jobId] || CAR_REPAIR_IMAGES_MAP["car_repair_1"];
    }, [job]);
    ...
    // Get car repair name from job data - Using useCallback
    const getName = useCallback((): string => {
        if (!job) return "Car Repair Service";
        return job.title || "Car Repair Service";
    }, [job]);
    ...
    const getDescription = useCallback((): string => {
        if (!job) return "Professional car repair and maintenance services";
        const jobId = job.id || "car_repair_1";
        return (
            CAR_REPAIR_DESCRIPTIONS_MAP[jobId] ||
            job.description ||
    ...
    // Get phone number for the car repair service - Using useCallback
    ...
    const handleImageError = useCallback(() => {
        console.warn("⚠️ Failed to load image for car repair:", job?.id || "unknown");
        setImageError(true);
    }, [job]);
    ...
    const visibleServices = CAR_REPAIR_SERVICES.slice(0, 4);
    const moreServices =
        CAR_REPAIR_SERVICES.length > 4 ? CAR_REPAIR_SERVICES.length - 4 : 0;
...
const NearbyCarRepairCard: React.FC<NearbyCarRepairCardProps> = (props) => {
    // If no job is provided, render the grid of dummy car repairs
    if (!props.job) {
        return (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
                {DUMMY_CAR_REPAIRS.map((carRepair) => (
                    <SingleCarRepairCard
                        key={carRepair.place_id}
                        job={{
                            id: carRepair.place_id,
                            title: carRepair.name,
                            location: carRepair.vicinity,
                            distance: carRepair.distance,
                            category: "Car Repair Service",
                            jobData: {
                                rating: carRepair.rating,
                                user_ratings_total: carRepair.user_ratings_total,
                                opening_hours: carRepair.opening_hours,
                                geometry: carRepair.geometry,
                                business_status: carRepair.business_status,
                                price_level: carRepair.price_level,
                                types: carRepair.types,
                                special_tags: carRepair.special_tags,
                            },
                        }}
    ...
    return <SingleCarRepairCard job={props.job} onViewDetails={props.onViewDetails} />;
};

export default NearbyCarRepairCard;