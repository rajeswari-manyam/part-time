...
// Phone numbers map for hospitals
...
// Export dummy hospital data - EXACT FROM REACT NATIVE
export const DUMMY_HOSPITALS: HospitalService[] = [
  {
    place_id: "hospital_1",
    name: "Lalitha Hospital",
    vicinity: "Gajularamaram Kukatpally, Hyderabad",
    rating: 4.6,
    user_ratings_total: 383,
    photos: [{ photo_reference: "hospital_photo_1" }],
    geometry: {
      location: { lat: 17.5, lng: 78.41 },
    },
  ...
  },
  {
    ...
    photos: [{ photo_reference: "hospital_photo_2" }],
    geometry: {
  ...
  },
  {
    ...
    vicinity: "Sindhi Colony-secunderabad, Secunderabad",
    rating: 3.8,
    user_ratings_total: 635,
    photos: [{ photo_reference: "hospital_photo_3" }],
    geometry: {
      location: { lat: 17.44, lng: 78.495 },
    },
    business_status: "OPERATIONAL",
    opening_hours: { open_now: true },
    price_level: 2,
  ...
  },
  {
    ...
    photos: [{ photo_reference: "hospital_photo_4" }],
    geometry: {
      location: { lat: 17.538, lng: 78.49 },
    },
    business_status: "OPERATIONAL",
    opening_hours: { open_now: true },
    price_level: 2,
...
// Hospital images - Medical facility images
const HOSPITAL_IMAGES_MAP: { [key: string]: string[] } = {
  hospital_1: [
...
const HOSPITAL_DESCRIPTIONS_MAP: { [key: string]: string } = {
  hospital_1:
    "Multi-specialty hospital with 383+ ratings. Top search result. Open 24 Hours. 2 Years in Healthcare. Trust verified. Quick Response available. Book appointment online.",
  hospital_2:
    "Highly rated multi-specialty hospital with 2,203+ ratings. Responsive service. Open 24 Hours. 9 Years in Healthcare. Located 1 km from your location. Online appointments available.",
  hospital_3:
    "Homeopathic specialist hospital with 635+ ratings. Top Search result. Open 24 Hours. 17 Years in Healthcare. Trust verified. Book appointment available.",
  hospital_4:
    "Responsive multi-specialty hospital with 502+ ratings. Open 24 Hours. 9 Years in Healthcare. Located 2 km away. Online enquiry available.",
};
...
// Single Hospital Card Component
const SingleHospitalCard: React.FC<{ job: JobType; onViewDetails: (job: JobType) => void }> = ({
  job,
  onViewDetails,
}) => {
  ...
  const getPhotos = useCallback(() => {
    if (!job) return [];
    const jobId = job.id || "hospital_1";
    return HOSPITAL_IMAGES_MAP[jobId] || HOSPITAL_IMAGES_MAP["hospital_1"];
  }, [job]);
  ...
  const getName = useCallback((): string => {
    if (!job) return "Hospital";
    return job.title || "Hospital";
  }, [job]);
  ...
  const getDescription = useCallback((): string => {
    ...
    const jobId = job.id || "hospital_1";
    return (
      HOSPITAL_DESCRIPTIONS_MAP[jobId] ||
      job.description ||
  ...
  const getOpeningStatus = useCallback((): string | null => {
    ...
    return isOpen ? "Open 24 Hrs" : "Closed";
  }, [job]);

  // Get phone number for the hospital - Using useCallback like React Native
  ...
  const handleImageError = useCallback(() => {
    console.warn("⚠️ Failed to load image for hospital:", job?.id || "unknown");
    setImageError(true);
  }, [job]);
  ...
  const moreServices =
    HOSPITAL_SERVICES.length > 4 ? HOSPITAL_SERVICES.length - 4 : 0;
  ...
  return (
    ...
    >
      ...
      <div className="p-3.5">
        ...
        {/* Category Badge */}
        <div className="inline-flex items-center gap-1 bg-red-100 text-red-500 text-[11px] font-semibold px-2 py-1 rounded-xl mb-2">
          <span className="text-sm">🏥</span>
          <span>{job.category || "Hospital"}</span>
        </div>
        ...
        <div className="mb-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {visibleServices.map((service, index) => (
              <span
                key={index}
                className="inline-flex items-center gap-1 bg-red-100 text-red-500 text-[11px] font-medium px-2 py-1 rounded"
              >
                <CheckCircle size={11} />
                <span>{service}</span>
              </span>
...
const NearbyHospitalCard: React.FC<NearbyHospitalCardProps> = (props) => {
  // If no job is provided, render the grid of dummy hospitals
  if (!props.job) {
    return (
          ...
          <SingleHospitalCard
            ...
            job={{
              ...
              jobData: {
                ...
                price_level: hospital.price_level,
  ...
  return <SingleHospitalCard job={props.job} onViewDetails={props.onViewDetails} />;
};
