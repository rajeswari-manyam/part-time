...
// Import existing event card components
...
const EventServicesList: React.FC = () => {
    ...
    const handleView = (event: any) => {
        navigate(`/event-services/details/${event.id}`);
    };

    const handleEdit = (id: string, e: React.MouseEvent) => {
        e.stopPropagation();
        navigate(`/add-event-service-form/${id}`);
    };
    ...
    const handleAddPost = () => {
        navigate("/add-event-service-form");
    };
    ...
    const getCardComponentForSubcategory = (
        ...
        // ✅ PARTY DECORATIONS MATCHING
        if (
            normalized.includes("party") && normalized.includes("decoration") ||
            normalized.includes("party-decoration")
        ) {
            console.log("✅ Matched to NearbyPartyDecorationCard");
            return NearbyPartyDecorationCard;
        }

        // ✅ MANDAP DECORATIONS MATCHING
        if (
            normalized.includes("mandap") ||
            normalized.includes("wedding") && normalized.includes("decoration")
        ) {
            console.log("✅ Matched to NearbyMandapCard");
            return NearbyMandapCard;
        }
                                ...
                                {services.map((service) => (
                                    ...
                                    >
                                        ...
                                        <div className="absolute top-3 left-3 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-sm font-semibold flex items-center gap-1 z-10">
                                            <span>{service.eventData?.icon || "🎉"}</span>
                                        ...
                                        <div className="p-4 space-y-2">
                                            ...
                                            {service.eventData?.pincode && (
                                                <div className="flex items-center gap-2 text-sm">
                                                    <span className="text-gray-400">📍</span>
                                                    <span className="text-gray-700">
                                                        Pincode: {service.eventData.pincode}
                                                    </span>
                                                </div>
                                            )}

                                            {service.eventData?.status !== undefined && (
                                                <div className="flex items-center gap-2">
                                                    <span
                                                        className={`inline-flex items-center text-xs font-semibold px-2.5 py-1 rounded-full ${
                                                            service.eventData.status
                                                    ...
                                                    >
                                                        ...
                                                        {service.eventData.status ? "Available" : "Busy"}
                                                    </span>
                                                </div>
                                            )}
                                        </div>
