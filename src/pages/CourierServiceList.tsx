...
// Import courier card components
...
const CourierServicesList: React.FC = () => {
    ...
    const [services, setServices] = useState<CourierServiceType[]>([]);
    const [loading] = useState(false);
    const [error] = useState("");

    const handleView = (courier: any) => {
        navigate(`/courier-services/details/${courier.id}`);
    };

    const handleEdit = (id: string, e: React.MouseEvent) => {
        e.stopPropagation();
        navigate(`/add-courier-service-form/${id}`);
    };
    ...
    const handleAddPost = () => {
        navigate("/add-courier-service-form");
    };
    ...
    const getCardComponentForSubcategory = (
        ...
        // ✅ COURIER OFFICE MATCHING
        if (
            normalized.includes("courier") && !normalized.includes("parcel") && !normalized.includes("packer")
        ) {
                                ...
                                {services.map((service) => (
                                    ...
                                    >
                                        ...
                                        <div className="absolute top-3 left-3 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-sm font-semibold flex items-center gap-1 z-10">
                                            <span>{service.courierData?.icon || "📦"}</span>
                                        ...
                                        <div className="p-4 space-y-2">
                                            ...
                                            {service.courierData?.pincode && (
                                                <div className="flex items-center gap-2 text-sm">
                                                    <span className="text-gray-400">📍</span>
                                                    <span className="text-gray-700">
                                                        Pincode: {service.courierData.pincode}
                                                    </span>
                                                </div>
                                            )}

                                            {service.courierData?.status !== undefined && (
                                                <div className="flex items-center gap-2">
                                                    <span
                                                        className={`inline-flex items-center text-xs font-semibold px-2.5 py-1 rounded-full ${
                                                            service.courierData.status
                                                    ...
                                                    >
                                                        ...
                                                        {service.courierData.status ? "Available" : "Busy"}
                                                    </span>
                                                </div>
                                            )}
                                        </div>
