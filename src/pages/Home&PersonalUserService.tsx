...
const HomePersonalUserService: React.FC<HomePersonalUserServiceProps> = ({
    ...
    useEffect(() => {
        const fetchJobs = async () => {
            ...
            try {
                ...
                // Filter home category jobs - handle multiple possible category names
                const homePersonalJobs = (response.jobs || []).filter((job: any) => {
                    if (!job.category) return false;

                    const category = job.category.toLowerCase();

                    // Match various possible category names
                    return category === 'home' ||
                        category === 'home-personal' ||
                        category === 'homepersonal' ||
                        category.includes('home');
                });

                console.log('📊 Total jobs from API:', response.jobs?.length || 0);
                console.log('📊 Filtered home jobs:', homePersonalJobs.length);

                setJobs(homePersonalJobs);
            } catch (error) {
    ...
    const filteredJobs = selectedSubcategory
        ? jobs.filter(job =>
            job.subcategory &&
            job.subcategory.toLowerCase().includes(selectedSubcategory.toLowerCase())
        )
        : jobs;
    ...
    // ── Helper functions ──
    const openDirections = (job: HomePersonalJob) => {
                    ...
                    {deletingId === id ? (
                        ...
                        <ActionDropdown
                            onEdit={(e?: React.MouseEvent) => {
                                e?.stopPropagation();
                                navigate(`/add-home-service-form?id=${id}`);
                            }}
